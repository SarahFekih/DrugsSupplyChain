...
const getCCP = async (org) => {
    ...
    if (org == "Org1MSP") {
        ...
        ccpPath = path.resolve(__dirname, '..', '..', 'test-network', 'organizations', 'peerOrganizations', 'org3.example.com', 'connection-org3.json');

    } else if (org == "Org4MSP") {
        ccpPath = path.resolve(__dirname, '..', '..', 'test-network', 'organizations', 'peerOrganizations', 'org4.example.com', 'connection-org4.json');

    } else if (org == "Org5MSP") {
        ccpPath = path.resolve(__dirname, '..', '..', 'test-network', 'organizations', 'peerOrganizations', 'org5.example.com', 'connection-org5.json');
    } else
        return null
...
const getUser =  async(user,mspid) => {

    const walletPath = await getWalletPath(mspid);
       var  data=fs.readFileSync(`${walletPath}/${user}.id`)
        var JsonData = JSON.parse(data.toString())
        var mspid = JsonData.mspId
        var _password =JsonData.password
    return [JsonData.mspId,_password]
}
